...
这是本方案最重要的前提变化。下表逐项列出常规实时约束下被放弃、而现在重新可行的技术：

| 技术 | 常规 30–50 Hz 下 | 10 Hz（100 ms 预算）下 | 依据 |
|---|---|---|---|
| **闭环生成未来视频/4D 再产生动作** | 不可行（数百 ms 到数秒） | **可行**：DreamZero 14B 骨干做到 7 Hz 闭环；GigaWorld-Policy 360 ms；Fast-WAM 190 ms | [DreamZero](https://dreamzero0.github.io/DreamZero.pdf)、[鲁棒性对比研究](https://arxiv.org/html/2603.22078v2) |
| **测试时扩展**（采样 N 个候选 → 世界模型 rollout 打分 → 选最优） | 完全没预算 | **可行**：N 个候选可 batch 并行，算力不限时几乎不增加墙钟时间 | §5.4 |
| **大骨干**（14B+ 视频扩散 / 7B+ VLM） | 通常上限 2–3B | **可行**，DreamZero 证明大骨干带来 2× 以上的泛化提升 | [DreamZero](https://dreamzero0.github.io/DreamZero.pdf) |
| **推理时输出完整稠密 4D**（不只是训练时监督） | 必须"训练时挂头、推理时丢头" | **可以常开**，直接满足"4D 信息越多越好"的需求 | §4.2 |
| **多步扩散去噪**（而非蒸馏到 1–2 步） | 必须蒸馏 | 可用 10–50 步，动作质量更好 | — |
| **在线增量 3D 重建 / 持久空间记忆** | 通常放弃 | **可行**，对移动本体尤其重要 | §3.6 |
| **同时跑多个 4D 表征头**（深度+流+点轨迹+占据+高斯） | 只能挑一两个 | **全都要** | §4.2 |

**一句话**：常规 VLA 方案里大部分架构妥协都是为了实时性。你把这个约束放宽 3–5 倍并去掉算力上限之后，应当**主动选择更重的架构**，而不是沿用轻量方案。

**但有两个前提不会因为算力而消失**：

1. **10 Hz 意味着 100 ms 的感知-动作延迟下限。** 对精细接触任务（插拔、对孔），这是真实的物理限制。解决办法不是提频，而是**动作块 + 时序集成 + 底层高频控制器**：顶层 10 Hz 给出未来 1–5 s 的意图，底层 200 Hz+ 的控制器负责跟踪与局部纠偏。这是 §5 推荐三层架构的根本原因。
2. **动作块拼接的抖动问题不会因为算力消失。** 需要 [Real-Time Chunking (RTC)](https://www.pi.website/research/real_time_chunking) 或 LeRobot 的 `weighted_average` 之类机制。[OpenHLM](https://arxiv.org/abs/2606.22174) 给了一个相关数据点：全身控制器的**未来帧预览延迟 Δt = 0.2 s 是最优点**——Δt=0 时移动顿挫、踏地，Δt=0.6 s 时演示时长从 35 s 涨到 41 s、任务进度崩到 13%。
...
| **端到端 vs 分层**（本设定新增） | 一个模型直出全身关节命令 | VLA 出中层指令 + 低层控制器执行 | — | **分层**。[WholeBodyVLA](https://arxiv.org/html/2512.11047v1) 与 [OpenHLM](https://arxiv.org/abs/2606.22174) 都用三层；但 OpenHLM 同时指出"whole-body native"的端到端做法在垂直大范围任务上更强，两者要按任务类型取舍（§5.5） |
...
| 方法 | 核心做法 | 关键数字 | 开源 |
|---|---|---|---|
| [**Co-VLA**](https://arxiv.org/html/2606.20285v1) | 把 monolithic 动作头换成 **Structured Action Expert (SAE)**：动作分解为 **shared latent**（任务级协同意图）+ **per-arm residual latent**（各臂执行微调）；配 **Latent-Aware Controller (LAC)** 在部署时按 latent 调节同步强度、平滑度与安全约束 | 紧耦合任务成功率 **+27 个百分点**；真机 OOD 从 13%→27%（翻倍以上）；完成时间 **−25%**；LAC 不需要力控或阻抗控制，兼容标准关节命令管线 | 论文 |
| [**TwinVLA**](https://arxiv.org/html/2511.05275v1)（ICLR 2026） | 把**两份预训练单臂 VLA** 用 **joint attention** 组合成双臂策略，只复制 VLM 骨干、用 MoE 管理共享输入 | **无需任何双臂预训练**即超过同规模 RDT-1B；逼近用大量私有双臂数据训练的 π0 | [GitHub](https://github.com/jellyho/TwinVLA) |
| **SkillVLA** | 两级推理：先判断"这一步是否需要双臂协作"，再复用/重组单臂技能 | 组合泛化 | — |
| **CoFreeVLA** | 增加**短时程自碰撞风险估计器**，实时门控不安全命令并引导恢复到安全状态 | 专门解决双臂自碰撞 | — |
...
1. **动作头必须结构化**。最低限度是"共享 + 残差"的两级分解；这不仅提升成功率，还让失败可归因（是协同计划错了，还是单臂执行错了）。Co-VLA 论文的原话是"把双臂动作表示成一个整体向量，混淆了两种本质不同的变异来源"。
2. **协同类型要分层处理**。建议在数据标注与损失设计上区分四类：独立并行（两手做不相干的事）、松耦合（一手扶一手做）、紧耦合（两手同时作用于同一物体，但不构成刚性闭链）、**闭链约束**（两手刚性抓同一刚体，形成闭运动链，此时两手位姿之间存在硬约束 $T^{\text{ee,L}} \cdot T_{\text{rel}} = T^{\text{ee,R}}$）。闭链是最难的，也是最能从"显式约束损失"中受益的。
3. **自碰撞要单独建模，不要指望端到端学会**。双臂 + 躯干的构型空间里自碰撞区域很大，示教数据里几乎没有负样本。做法：用 URDF 做解析自碰撞检测生成海量负样本，训一个轻量的"未来 H 步自碰撞风险"分类头，在动作输出前门控（CoFreeVLA 思路）。**这个头天然属于 4D 输出的一部分**——它就是对预测出的本体 4D 做碰撞查询。
4. **组合式初始化值得考虑**。TwinVLA 证明"两份单臂 VLA + joint attention"能绕开双臂数据稀缺。考虑到公开单臂数据（OXE、DROID）远多于双臂数据，这条路对你有实际价值。
...
1. **三层架构，顶层就是 10 Hz。** 你的 10 Hz 指标恰好与 WholeBodyVLA 的 VLA 层频率一致，这不是巧合——它是"VLM 前向 + 动作块解码"的自然工作点。底层控制器该跑 50–500 Hz 就跑，与顶层解耦。
2. **基座命令用离散接口。** 这是本节最具体、代价最低、收益最明确的一条。把基座动作离散成 {前进 x cm、后退、左右侧移、原地左右转 θ 度、升降躯干、停止} 这类原语，而不是回归连续的 $(v_x, v_y, \omega_z)$。WholeBodyVLA 的对照实验给出了 24 个百分点的差距。**注意**：这与"动作块"并不冲突——动作块里基座那一维可以是离散 token 序列，双臂那部分仍是连续值。
3. **loco 与 mani 的表征要分离。** 前面 §2.7 已详述。对 4D 头同样适用。
4. **"基座该停在哪"值得作为一个独立的预测目标。** Mobi-π 把它形式化为 mobilization 问题。在你的方案里，这可以是 4D 输出的一个附加层：预测**基座候选落位的可达性热力图**——给定当前观测与任务，哪些基座位姿能让双臂够到目标。这既是有用的 4D 信息，也是直接的决策依据。
5. **π0.5 初始化 + 轮式双臂预训练数据是你的最短路径。** OpenHLM 的实证说"在静态与轮式双臂平台上预训练的 VLA 迁移到全身动作空间表现出奇地好"——而你的本体就是轮式双臂，比人形更接近预训练分布，迁移应当更容易。
6. **评测别用 action MSE。** OpenHLM 明确指出这是个糟糕的代理指标，并改用**任务进度分数**（$[0,1]$ 的分段给分）而非二值成功率。这条已写入 §8。
...
> **建议组合**：(a) + (b) 作为主路，(c) 的基座系俯视图作为**第四路"虚拟相机"**输入。俯视图对双臂协同的空间布局判断、以及基座落位判断特别有用，且它不随头部转动而变化，等于人为造了一个"固定第三人称视角"。
...
4. **头部朝向作为动作输出的一部分**。既然头部可动，"看哪里"就是一个决策。把 $q^{\text{neck,cmd}}$ 纳入动作块，让模型学会主动注视目标。这一条有明确的相关工作方向（Vision-in-Action 等主动视觉 VLA），而且在 4D 侧有一个漂亮的自监督信号：**预测"如果我把头转到某角度会看到什么"**——这是一个完全免费的 4D 监督（用后续实际转头看到的帧作真值）。
5. **腕部相机的自遮挡必须显式处理**。腕部相机画面里往往 30–50% 是夹爪本身。用 §3.3 第 2 条的解析掩膜把这些像素标出来，在深度/光流损失里排除，同时作为输入通道告诉模型"这块是我的手"。
...
│       ├── T_ee[H, 2, 9]           双臂末端位姿（相对当前帧，3 位置 + 6D 旋转）
│       ├── T_rel[H, 9]             双臂相对位姿（闭链任务的核心量）
│       └── gripper[H, 2]           双夹爪
│
├── 基座命令（离散 token 自回归头）
│   └── u_base[H]  ∈ {停止, 前进 Δd, 后退 Δd, 左移, 右移, 左转 Δθ, 右转 Δθ, ...}
│
├── 躯干 / 头部（连续，小头）
│   ├── q_torso[H, n_t]             升降 / 俯仰
...
**另一条可选路线**是 [TwinVLA](https://github.com/jellyho/TwinVLA) 的组合式：两份预训练**单臂** VLA + joint attention + MoE 管理共享输入。它的价值在于**绕开双臂数据稀缺**——公开单臂数据（OXE、DROID）比双臂数据多一个数量级。如果你的自采双臂数据不多，这条路值得认真考虑。两条路线也可以结合：TwinVLA 式的初始化 + Co-VLA 式的共享/残差结构。
...
| **闭链一致性** | 双手同时抓同一刚体时，$T^{\text{ee,L}\leftarrow\text{ee,R}}$ 恒定 | 与 §4.1.4 的动作侧约束呼应，形成动作-4D 双向绑定 |
...
**为什么 10 Hz 下这件事变得可行**：N 个候选动作块可以在 batch 维度**完全并行**，算力不限时墙钟时间几乎不随 N 增长。100 ms 预算里，跑一次 N=32 的批量前向 + 解析碰撞检查是宽裕的。

**最低限度的建议**：即使不做完整的 MPC，也**一定要做"解析安全检查 + 拒绝采样"**。这一条几乎零成本（本体 4D 反正要算），却能直接消除一大类硬失败。具体做法：采样 8 个候选动作块 → 对每个算 swept volume → 与场景占据和自身求交 → 丢弃有碰撞的 → 在剩下的里选 value 最高的。如果全部都有碰撞，退回到"停止 + 重新规划"。
...
└────────────────────────┬─────────────────────────────────────┘
         ...
         ▼               ▼                            ▼
┌─────────────────┐ ┌──────────────────┐  ┌─────────────────────────┐
│ 慢 4D 世界分支   │ │ 双臂结构化动作专家 │  │ 基座 / 躯干 / 头部命令头 │
│  1~3 Hz          │ │  10 Hz            │  │  10 Hz                   │
│ 三分支 DiT       │ │ shared latent     │  │ 离散原语 AR 解码          │
│ RGB / D / Flow   │ │  + L/R residual   │  │ + 躯干俯仰升降            │
│ 条件：**解析的   │ │ flow matching     │  │ + 颈部朝向（主动视觉）    │
│ 未来相机外参**   │ │ 5~20 步去噪       │  └─────────────────────────┘
│ ├ 头部头：场景级 │ │                   │
│ └ 腕部头：接触级 │ │ 读慢分支 latent    │
└────────┬────────┘ │  (cross-attention) │
...
| 双臂动作头 | **Co-VLA 式 SAE**：shared coordination latent + per-arm residual，flow matching 解码 | TwinVLA 式双单臂 + joint attention | 紧耦合任务 +27 个百分点；失败可归因。若自采双臂数据少，改用 TwinVLA 路线绕开数据瓶颈 |
...
| 点图/位姿 | VGGT / DA3 Nested（度量尺度） | CUT3R / StreamVGGT（在线流式） | 算力不限时可在线常开，用于校验标定与补全深度 |
...
- 损失项这么多，**梯度冲突是真实风险**。建议用不确定性加权（Kendall 式可学习 $\log\sigma^2$）或 PCGrad，而不是手调 20 个权重。至少要做的是：把损失按上表分组，组内固定相对权重，只调 5 个组间权重。
...
```
时间轴（顶层 10 Hz = 100 ms 周期；底层控制器 50~500 Hz 独立运行）

t=0 ms   ├─ 采集三路图像 + 全身状态 + 里程计
         ├─ 解析计算：逐帧 ray map、ego-flow、本体掩膜、自碰撞距离   (~2 ms)
         ├─ 记忆检索：把世界系记忆投影到当前基座系                  (~3 ms)
         │
t=5 ms   ├─ [主干] VLM 前向 → prefix                                (~20 ms)
         │
t=25 ms  ├─ [并行 A] 动作专家采样 N=8~32 个候选动作块               (~30 ms, batch 并行)
         ├─ [并行 B] L3 3D 点轨迹头（单次前向）                     (~5 ms)
         ├─ [并行 C] 慢 4D 分支（每 3~10 个周期跑一次，异步不阻塞）
         │            → L1/L2 未来 RGB-D-Flow，L4 占据 / 4DGS
         │
t=55 ms  ├─ 对 N 个候选：FK + swept volume + 自碰撞 + 场景占据求交   (~10 ms, 纯几何)
         ├─ value 头打分 → 选出最优候选                             (~5 ms)
         │
t=70 ms  ├─ 与上一块做 RTC / weighted_average 融合
         ├─ 下发到中层：双臂关节命令 + 基座离散原语 + 躯干/颈部
         │
t=100ms  └─ 下一周期；底层控制器在此期间以 50~500 Hz 跟踪
...
你的本体是"可动基座 + 双臂 + 头部/双腕相机"，**匹配度最高的公开数据集是 AgiBot World**（头部 + 双腕的相机配置与你完全一致），而不是常被推荐的 DROID（单臂 + 固定第三人称）。

| 数据集 | 规模 | 相机配置 | 双臂 | 可动基座 | 深度 | 与本设定匹配度 |
|---|---|---|---|---|---|---|
| [**AgiBot World**](https://agibot-world.com/)（GO-1） | 3000+ 小时 | **头部 + 双腕**（与你一致） | ✅ | 部分型号有 | ✅ | **最高**。Genie Envisioner 与 WholeBodyVLA 的 mani LAM 都用它 |
| [Open X-Embodiment](https://robotics-transformer-x.github.io/) | ~100 万+ 轨迹，22 种本体 | 混杂 | 部分 | 少数 | 少数子集 | 中。跨本体预训练的事实标准，子集质量参差需筛选 |
| [DROID](https://droid-dataset.github.io/) | 7.6 万条 / 350 小时 / 564 场景 | 2 路固定 ZED + 1 腕部 | ✗ | ✗ | ✅ 立体 | 中。**相机配置与你相反**（它有固定视角、你没有），但场景多样性极好，适合做泛化预训练 |
| RH20T | 11 万条 | 多视角 | 部分 | ✗ | ✅ RGB-D | 中 |
| RoboMIND | 10 万+ | 多视角 | ✅ | ✗ | ✅ | 中高 |
| Mobile ALOHA 数据 | 小 | 头部 + 双腕 | ✅ | **✅ 轮式** | ✗ | **配置匹配度高但规模小**，适合做 sanity check |
| Galaxea Open-World Dataset | — | 头部 + 双腕 | ✅ | **✅ 轮式双臂** | — | 本体形态与你最接近的公开数据之一 |
| RoboTwin 2.0 | 仿真 | 可配置 | ✅ | 部分 | ✅ 完美真值 | **高**。双臂基准，WAM 论文常用；可导出完美 4D 真值 |
| BEHAVIOR-1K / Habitat 3.0 / OVMM | 仿真 | 可配置 | 部分 | ✅ | ✅ | 移动操作的仿真来源 |
| Ego4D / Ego-Exo4D / EgoDex / HOI4D | 数千小时 | 第一视角（+ Ego-Exo4D 有外部视角） | 人类双手 | 人在走动 | 部分 | **4D 与潜动作预训练的主力**；人类双手数据天然是"双臂" |

**几条实务判断：**

1. **AgiBot World 优先**。相机配置一致意味着你可以直接复用它训 4D 头与 mani LAM，而不需要做视角适配。
2. **DROID 仍然值得用，但用法不同**。它有固定视角，正好可以用来**训练"从固定视角学到的场景理解"**，再通过 embodiment token 迁移到你的全动态设定。另外它自带立体深度，是伪标签流水线的绝佳调试数据。
3. **人类第一视角视频对你的价值比对单臂机器人更大**。原因有二：①人天然是双手协同的，人类视频里充满了双臂协同的自然示范；②人在操作时会走动、会转头，视觉模式与你的全动态相机高度相似——而固定相机的机器人数据完全没有这个特性。
4. **"操作感知移动"数据必须自采**（§6.4 已述）。公开数据里导航归导航、操作归操作，中间那段"带着操作意图接近工作台"的数据是空白。
5. **仿真是唯一能给完美 4D 真值的来源**，对校准 4D 头至关重要（§7.4）。
...
| C4 | 32 | 完整 MPC/CEM 多轮精化 | 上限探索 |

**每个变体都要报告**：任务进度分数（分双臂类型、分移动/静止）、样本效率曲线（10%/25%/50%/100% 数据）、自碰撞发生率、推理延迟、显存。
...
| 议题 | 要点 |
|---|---|
| **多损失平衡** | §6.3 有 20 项损失，手调权重不现实。用不确定性加权（可学习 $\log\sigma^2$）或 PCGrad；按组固定组内相对权重，只调 5 个组间权重；分阶段开启而非一次全开 |
...
| Co-VLA | 双臂共享/残差动作头 | 论文；SAE + LAC 的设计需自行实现 |
...
| R16 | **多损失梯度冲突**，20 项损失互相拉扯 | 中 | 中 | 不确定性加权 / PCGrad；分组调参；分阶段开启 |
...
- SkillVLA（单臂技能组合泛化到双臂）、CoFreeVLA（双臂自碰撞风险门控）、Dexora（高自由度双臂+灵巧手开源系统）
- RDT-1B（Robotics Diffusion Transformer，双臂基线）
- ACT / ALOHA / Mobile ALOHA（动作块与双臂遥操作的开山工作）
- RoboTwin 2.0（双臂基准）— [GitHub](https://github.com/RoboTwin-Platform/RoboTwin)
...
| A2 | 双臂动作头用**共享 + 残差**分解 | [Co-VLA](https://arxiv.org/html/2606.20285v1)：紧耦合任务 +27 个百分点，OOD 翻倍 | 若用 monolithic 向量，紧耦合与闭链任务会明显更差，且失败无法归因 |
...
| A8 | 10 Hz 下**直接上闭环生成 4D**，不先做轻量版 | DreamZero 7 Hz、Fast-WAM 190 ms、GigaWorld 360 ms 已进入可用区间 | 若判断错（实际延迟不达标），退回方案 A——但方案 A 的组件全部可复用，代价可控 |
...
| P6 | 需要商用许可 | 优先 Apache 2.0（GR00T N1.7、openpi、DA3）、MIT（StarVLA、OpenVLA、TwinVLA）；逐一登记每个组件 |
